...
# ... código de add_child(nuevo_edificio) ...

	# INICIO DE ANIMACIÓN POR CÓDIGO (TWEEN)
	var altura_inicial = 20.0 # Cae desde 20 metros
	nuevo_edificio.position.y = altura_inicial 
	
	var tween = get_tree().create_tween()
	# Animamos la posición Y desde 20 hasta 0 en 1.5 segundos con efecto de rebote
	tween.tween_property(nuevo_edificio, "position:y", 0.0, 1.5)\
